...
export default function CodeBlock({
  ...
  return (
    <figure className="flex flex-col gap-1 py-3">
      ...
      <pre className="h-auto bg-button-100 text-sm rounded-sm font-mono p-4 selection:bg-background! overflow-x-auto flex flex-col custom-hor-scrollbar leading-[1.7em]">
        {code.map((child, idx) => (
          <code
            key={idx}
            className="text-text-900 h-auto min-h-[1.4em]"
