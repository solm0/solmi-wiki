...
export default function GraphController({
  ...
  const [postsById, setPostsById] = useState<Record<string, GraphPost>>({});

  const handleDepthChange = async (newDepth: number) => {
    if (!postId) return;
    const newGraph: Graph = { nodes: [], links: [] };
    await expandGraphToDepth(postId, postsById, newDepth, newGraph);
    setGraph(newGraph);
    setDepth(newDepth);
  };
  ...
  useEffect(() => {
    async function loadGraphPosts() {
      ...
      const nextPostsById = Object.fromEntries(posts.map((graphPost) => [graphPost.id, graphPost]));
  ...
  useEffect(() => {
    if (postId && Object.keys(postsById).length > 0) {
